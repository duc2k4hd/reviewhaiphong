...
class MiaScraperController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('admin.mia-scraper.index', compact('account'));
    }

    /**
     * Xử lý scraping từ URL mia.vn
     */
    public function scrape(Request $request)
    {
        ...
        try {
            ...
            foreach ($urls as $url) {
                ...
                if (!filter_var($url, FILTER_VALIDATE_URL) || !preg_match('/^https?:\/\/(www\.)?mia\.vn\/.+/i', $url)) {
            ...
            if (empty($validUrls)) {
                return back()->withErrors(['urls' => 'Không có URL hợp lệ nào. Vui lòng kiểm tra lại định dạng URL (phải là link từ mia.vn).']);
            }
            ...
            foreach ($validUrls as $index => $url) {
                try {
                    ...
                    if ($result) {
                        ...
                        Log::info("MIA Scraper: Bỏ qua URL vì slug đã tồn tại: {$url}");
                    } else {
                        $errorCount++;
            ...
            Log::error('MIA Scraper Error: ' . $e->getMessage(), [
    ...
    private function scrapeSingleUrl(string $url, $account)
    {
        try {
            ...
            // Lấy hoặc tạo category "Review Tổng Hợp"
            $category = Category::firstOrCreate(
                ['slug' => 'review-tong-hop'],
                [
                    'name' => 'Review Tổng Hợp',
                    'status' => 'active',
                    'sort_order' => 0
                ]
            );
            ...
            // Thay thế MIA.vn hoặc mia.vn thành Review Hải Phòng trong title
            $seoTitle = preg_replace('/\bMIA\.vn\b/i', 'Review Hải Phòng', $seoTitle);
            $seoTitle = preg_replace('/\bmia\.vn\b/i', 'Review Hải Phòng', $seoTitle);
            ...
            if (Post::where('slug', $slug)->exists()) {
                Log::info("MIA Scraper: Bỏ qua bài viết vì slug đã tồn tại: {$slug} - URL: {$url}");
            ...
            // Thay thế MIA.vn hoặc mia.vn thành Review Hải Phòng trong description
            $seoDesc = preg_replace('/\bMIA\.vn\b/i', 'Review Hải Phòng', $seoDesc);
            $seoDesc = preg_replace('/\bmia\.vn\b/i', 'Review Hải Phòng', $seoDesc);

            $data['seo_desc'] = $seoDesc;

            // 3. Cào content từ <div class="post-content">
            $contentNodes = $xpath->query('//div[@class="post-content"]');
            if ($contentNodes->length === 0) {
                // Thử tìm với contains class
                $contentNodes = $xpath->query('//div[contains(@class, "post-content")]');
            }
            
            if ($contentNodes->length === 0) {
                throw new \Exception('Không tìm thấy nội dung bài viết (div.post-content)');
            }
            ...
            // 4. Lấy ảnh đại diện từ <div class="post-top-img">
            $topImgNodes = $xpath->query('//div[contains(@class, "post-top-img")]/@style');
            $seoImage = null;
            
            if ($topImgNodes->length > 0) {
                $style = $topImgNodes->item(0)->value;
            ...
            // Nếu không có ảnh từ post-top-img, lấy ảnh đầu tiên từ content
            if (!$seoImage) {
                $firstImgNodes = $xpath->query('//div[@class="post-content"]//img[1]/@src');
            ...
            Log::error('MIA Scraper Single URL Error: ' . $e->getMessage(), [
                'url' => $url,
                'trace' => $e->getTraceAsString()
            ]);
            throw $e;
    ...
    /**
     * Xử lý content: chuyển div thành p, loại bỏ class/attributes, download ảnh, thay thế link "Xem thêm"
     */
    private function processContent($contentDiv, DOMDocument $dom, string $slug, string $title): string
    {
        ...
        // Thay thế text "MIA.vn" hoặc "mia.vn" thành "Review Hải Phòng" trong text nodes
        $textNodes = $newXpath->query('//text()');
        foreach ($textNodes as $textNode) {
            $text = $textNode->nodeValue;
            // Chỉ thay thế nếu không phải là URL (không chứa http:// hoặc https://)
            if (strpos($text, 'http://') === false && strpos($text, 'https://') === false) {
                $newText = preg_replace('/\bMIA\.vn\b/i', 'Review Hải Phòng', $text);
                $newText = preg_replace('/\bmia\.vn\b/i', 'Review Hải Phòng', $newText);
                if ($newText !== $text) {
        ...
        foreach ($paragraphs as $p) {
            ...
            if (preg_match('/xem\s+thêm/i', $pText)) {
                ...
                foreach ($links as $link) {
                    ...
                    // CHỈ thay thế nếu link trỏ đến mia.vn (kiểm tra chặt chẽ hơn)
                    if (preg_match('/https?:\/\/(www\.)?mia\.vn\//i', $href)) {
